...
class TrendUpdater:
    ...
    请基于**当前最新**的全球时政经济局势，列出需要重点监控的中文关键词。

    请识别以下三类热点（每类提取 3-8 个最核心的词）：
    1. **关键人物 (HIGH)**：对市场有巨大影响力的**现任**政治/经济领袖。请输出中文译名（如"特朗普"、"鲍威尔"）。
    2. **地缘热点 (HIGH)**：**当前正在进行**的、可能影响供应链或避险情绪的冲突/事件/地点。请输出中文名称（如"红海危机"、"台海紧张"）。
    3. **热门概念 (MEDIUM)**：**近期**市场炒作的科技或经济概念。请输出中文通用名称（如"低空经济"、"具身智能"）。
